...
Commandes utiles : help, ls, cd <dossier>, cat <fichier>, pwd, clear, date, whoami, etc.` },
    'college': { type: 'dir', content: {
      'README.txt': { type: 'file', content: 'Collège : découverte de l\'informatique avec la spécialité SNT (Sciences Numériques et Technologie). Initiation à Scratch, logique algorithmique, premiers pas en programmation.' }
    } },
    'lycee': { type: 'dir', content: {
...
Environnement : Microsoft Intune, Azure, Windows Server, AD, Wireshark, Nmap, Burp Suite, Gobuster.` }
  ...
  let currentPath = '/';  // chemin courant, simplifié pour l'exercice
  let currentDir = fs;    // on commence à la racine
  ...
  // Fonction pour afficher une ligne avec du HTML
  ...
  function processCommand(cmd) {
    ...
    // Commande secrète : 13/06/2006
    if (cmd.trim() === '13/06/2006') {
      printLine('La meilleur date au monde sahhh', '#ff69b4');
      return;
    }
    ...
    switch (command) {
      case 'help':
        ...
        printLine('  ls                      - Liste les fichiers/dossiers du répertoire courant');
        printLine('  cd <dossier>            - Change de répertoire (ex: cd college)');
      ...
      case 'cd':
        if (args.length === 0) {
          ...
          // remonter d'un niveau (simplifié : retour à la racine)
          currentPath = '/';
          currentDir = fs;
          printLine('Retour à la racine.');
        } else {
          ...
          if (currentDir.type === 'file') {
            ...
            currentPath += target + '/';
            printLine(`Vous êtes maintenant dans /${target}`);
          } else {
  ...
  // ----- 8. PETIT EFFET GLITCH SUPPLÉMENTAIRE -----
  setInterval(() => {
